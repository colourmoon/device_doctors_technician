...
bottombarAppbarWidget({
  required ServicesBottomBarCubit? servicesBottomBarCubit,
  required VoidCallback notificationOnTap,
}) {
  return AppBar(
    ...
    title: BlocBuilder<ProfileCubit, ProfileState>(
      builder: (context, state) {
        print("profile state : ${state.profileDetils} ${state.fetchLoading}");
        if (state.profileDetils != null) {
          return Row(
            children: [
              // profile image default image widget
              Padding(
                  padding:
                      const EdgeInsets.symmetric(horizontal: 5, vertical: 15),
                  child: state.profileDetils!.displayImage == ""
                      ? CircleAvatar(
                          radius: 22,
                      ...
                      : CircleAvatar(
                          ...
                          backgroundColor: Colors.grey.shade200,
                          backgroundImage:
                              NetworkImage(state.profileDetils!.displayImage))),
              3.pw,
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    CommonProximaNovaTextWidget(
                      text: state.profileDetils!.ownerName,
                      fontSize: 16,
        ...
        if (state.fetchLoading == true && state.profileDetils != null) {
          return Row(
            children: [
              // profile image default image widget
              Padding(
    ...
    actions: [
      InkWell(
        ...
        child: Stack(
          ...
          children: [
            ...
            BlocBuilder<NotificationsDataCubit, NotificationsDataState>(
              builder: (context, notiState) {
                if (notiState.notificationsDataNewResponseModel != null &&
                    notiState.notificationsDataNewResponseModel!.results
                        .isNotEmpty &&
                    notiState.notificationsDataNewResponseModel!.results
                        .any((result) => result.viewStatus != '1') ) {
                  return Positioned(
                    right: 10,
                    top: 18,
                    child: Container(
                      padding: const EdgeInsets.all(
                          2), // Adjust the padding to size the badge
                      decoration: BoxDecoration(
                        color: const Color(0xffBB001B), // Badge color
                        borderRadius:
                            BorderRadius.circular(8), // Circular badge
                      ),
      ...
      BlocProvider(
        create: (context) =>
            ProfileVerficationCubit()..profileVerificationCheck(),
        child: BlocListener<ProfileCubit, ProfileState>(
          listener: (context, state) {
            if (state.StatusUpdateSuccess == true) {
          ...
          child: BlocBuilder<ProfileCubit, ProfileState>(
            builder: (context, state) {
              if (state.profileDetils != null) {
                return BlocBuilder<ProfileVerficationCubit,
                    ProfileVerficationState>(
                  builder: (context, vstate) {
                    return Padding(
                        padding: const EdgeInsets.only(
                            right: 10, top: 14, bottom: 12),
                        child: InkWell(
                          onTap: () {
                            context
                            ...
                            // print(
                            // "verification status : ${vstate.verificationModel!.vendorStatus}");
                            if (vstate.verificationModel!.vendorStatus != "0") {
                              context.read<ProfileCubit>().updateStatus(
                                  status: state.profileDetils!.isOnline == "0"
                                      ? "1"
                                      : "0");
                              // profilecubit.updateStatus(
                              //     status: state.profileDetils!.isOnline == "0"
                              //         ? "1"
                              //         : "0");
                            } else {
                              CommonSuccessToastwidget(
                                  toastmessage:
                                      "Please Wait Untill Your Profile Is Verified");
                            }
                          },
                          child: Container(
                            height: 18,
                            ...
                            decoration: BoxDecoration(
                                ...
                                color: state.profileDetils!.isOnline == "0"
                                    ? Colors.red
                                    : AppthemeColor().greenButtonColor),
                            child: Padding(
                              padding: state.profileDetils!.isOnline == "0"
                                  ? const EdgeInsets.only(
                                      right: 8, top: 5, bottom: 4, left: 2)
                                  : const EdgeInsets.only(
                                      left: 8, top: 5, bottom: 4, right: 2),
                              child: state.profileDetils!.isOnline == "0"
                                  ? Row(
                                      children: [
...
commonAppBarWidget(
    {required VoidCallback leadingArrowOnTap,
